...
func TestZonalGKECluster(t *testing.T) {
	t.Parallel()
	...
	test_structure.RunTestStage(t, "create_test_copy_of_examples", func() {
		...
		terraformModulePath := filepath.Join(testFolder, "gke-public-cluster")
		test_structure.SaveString(t, workingDir, "zonalGKETerraformModulePath", terraformModulePath)
	})

	test_structure.RunTestStage(t, "create_terratest_options", func() {
		zonalGKETerraformModulePath := test_structure.LoadString(t, workingDir, "zonalGKETerraformModulePath")
		...
		gkeClusterTerratestOptions := createTestGKEClusterTerraformOptions(t, uniqueID, project, zone, region, zonalGKETerraformModulePath)
		test_structure.SaveString(t, workingDir, "uniqueID", uniqueID)
		test_structure.SaveString(t, workingDir, "project", project)
		test_structure.SaveString(t, workingDir, "location", zone)
	...
	test_structure.RunTestStage(t, "configure_kubectl", func() {
		...
		// gcloud beta container clusters get-credentials example-cluster --zone australia-southeast1-a --project dev-sandbox-123456
		cmd := shell.Command{
			Command: "gcloud",
			Args:    []string{"beta", "container", "clusters", "get-credentials", clusterName, "--zone", location, "--project", project},
